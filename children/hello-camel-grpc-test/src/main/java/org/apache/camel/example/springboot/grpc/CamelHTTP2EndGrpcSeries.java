 ...
 */
@Component
public class CamelHTTP2EndGrpcSeries extends RouteBuilder {
	...
	//setup endpoint and its behavior
	    ...
	    //set the body with the object, using the interface with thr object and its type name
            public void process(Exchange exchange) throws Exception {
                ...
                .wireTap("seda:grpc://0.0.0.0:19501/middle")
                .to("mock:start");



        from("seda:grpc://0.0.0.0:19501/middle")

                .routeId("http2middles")
                .marshal()
                .mimeMultipart("mixed", true, true, "(included|x-.*)", true)
                //.to("log:org.apache.camel.example?level=INFO")
                .log("Message body after multipart: ${body}")
                .wireTap("seda:netty-http:http://0.0.0.0:18126/foo")
                .to("mock:middle");
                //.wireTap("mock:middle")
                //.log("Message body back multipart: ${body}");
               // ;

        from("seda:netty-http:http://0.0.0.0:18126/foo")
                .routeId("http2ends")
                .process(exchange -> {
                ...
                //.to("mock:end")
                .wireTap("seda:netty-http:http://0.0.0.0:18127/foo")
                .to("mock:ends");

        from("seda:netty-http:http://0.0.0.0:18127/foo")
                .routeId("http2endGrpcs")
                .process(exchange -> {
